...
server.get('/one/param/:first', function (req, res, next) {
  ...
  next();
}); // 4850 // ab -c 300 -n 200000 -H "accept: text/plain" http://localhost:3000/one/param/first
 
server.get('/two/params/:first/:second', function (req, res, next) {
  res.send(req.params.first + req.params.second);
  next();
}); // 4800 // ab -c 300 -n 200000 -H "accept: text/plain" http://localhost:3000/two/params/first/second
...
server.get('/three/params/:first/:second/:third', function (req, res, next) {
  ...
  next();
}); // 4800 // ab -c 300 -n 200000 -H "accept: text/plain" http://localhost:3000/three/params/first/second/third
